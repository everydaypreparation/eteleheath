...
export class InsurancesComponent implements OnInit {
  ...
  constructor(private router: Router,
    ...
    this.userService.setNav("insurances");
  }
      ...
      .subscribe(
        (res: any) => {
          try {
            ...
            console.log("Success Exception AdminInsuranceComponent getUserListByRoleName " + e);
          }
        ...
        (err: any) => {
          try {
            this.stickyBarService.hideLoader("");
            console.log("Error AdminInsuranceComponent getUserListByRoleName " + JSON.stringify(err));
            this.apiService.catchError(err);
          } catch (e) {
            console.log("Error Exception AdminInsuranceComponent getUserListByRoleName " + e);
          }
        }
      );
  }
  ...
  updateUser(action: string, userId: string) {
    if (action == 'add') {
      this.router.navigate([
        this.routeConfig.insuranceRegistrationPath
      ]);
    } else if (action == 'edit') {
      this.router.navigate([
        this.routeConfig.insuranceRegistrationPath, userId
      ]);
    }
  }
          ...
          .subscribe(
            (res: any) => {
              try {
                ...
                console.log("Success Exception InsurancesComponent deleteUserDetail " + e);
              }
              this.stickyBarService.hideLoader("");
            },
            (err: any) => {
              try {
                this.stickyBarService.hideLoader("");
                console.log("Error InsurancesComponent deleteUserDetail " + JSON.stringify(err));
                this.apiService.catchError(err);
              } catch (e) {
                console.log("Error Exception InsurancesComponent deleteUserDetail " + e);
              }
            });
      }
    });
