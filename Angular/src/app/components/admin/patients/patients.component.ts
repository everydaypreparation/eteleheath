...
export class PatientsComponent implements OnInit {
  ...
  constructor(private router: Router,
    ...
    this.userService.setNav("patients");
      ...
      .subscribe(
        (res: any) => {
          try {
            ...
            console.log("Success Exception AdminPatientComponent getUserListByRoleName " + e);
          }
        ...
        (err: any) => {
          try {
            this.stickyBarService.hideLoader("");
            console.log("Error AdminPatientComponent getUserListByRoleName " + JSON.stringify(err));
            this.apiService.catchError(err);
          } catch (e) {
            console.log("Error Exception AdminPatientComponent getUserListByRoleName " + e);
          }
        }
      );
  }
  ...
  updateUser(action: string, userId: string) {
    if (action == 'add') {
      this.router.navigate([
        this.routeConfig.patientRegistrationPath
      ]);
    } else if (action == 'edit') {
      this.router.navigate([
        this.routeConfig.patientRegistrationPath, userId
      ]);
    }
  }
          ...
          .subscribe(
            (res: any) => {
              try {
                ...
                console.log("Success Exception PatientsComponent deleteUserDetail " + e);
              }
              this.stickyBarService.hideLoader("");
            },
            (err: any) => {
              try {
                this.stickyBarService.hideLoader("");
                console.log("Error PatientsComponent deleteUserDetail " + JSON.stringify(err));
                this.apiService.catchError(err);
              } catch (e) {
                console.log("Error Exception PatientsComponent deleteUserDetail " + e);
              }
            });
      }
    });
