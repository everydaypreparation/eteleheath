...
export class PaymentComponent implements OnInit {
        ...
        (res: any) => {
          try {
            if (res.result.statusCode == 200) {
              ...
              //   this.routeConfig.patientPaymentDetailsPath, this.appointmentId, this.paymentId, this.doctorId,1
              // ]);
            } else {
              this.stickyBarService.showErrorSticky(res.result.message);
            }
        ...
        (res: any) => {
          try {
            ...
            if (res.result.statusCode == 200) {
              ...
              this.router.navigate([
                this.routeConfig.patientPaymentDetailsPath, this.appointmentId, this.paymentId, this.doctorId, verify
              ]);
            }
          } catch (e) {
        ...
        (res: any) => {
          try {
            if (res.result.statusCode == 200) {
              ...
              this.getUser(0);
              // this.router.navigate([
              //   this.routeConfig.patientPaymentDetailsPath, this.appointmentId, this.paymentId, this.doctorId,0
              // ]);
              //this.payAmount(0);
            } else {
              this.stickyBarService.showErrorSticky(res.result.message);
        ...
        (res: any) => {
          try {
            if (res.result.statusCode == 200) {
              ...
              this.originalAmount = res.result.items.filter(ue => ue !=null && ue.keyName=='Consultation Fee')[0].value;
              this.amount =  res.result.items.filter(ue => ue !=null && ue.keyName=='Consultation Fee')[0].value;
              }else{
  ...
  moveIntackForm(){
    this.router.navigate([this.routeConfig.patientAddInfoFormPath, this.appointmentId, this.doctorId]);
  }
