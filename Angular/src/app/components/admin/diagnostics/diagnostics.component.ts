...
export class DiagnosticsComponent implements OnInit {
  ...
  constructor(private router: Router,
    ...
    this.userService.setNav("diagnostics");
    }
      ...
      .subscribe(
        (res: any) => {
          try {
            ...
            console.log("Success Exception AdminDiagnosticComponent getUserListByRoleName " + e);
          }
        ...
        (err: any) => {
          try {
            this.stickyBarService.hideLoader("");
            console.log("Error AdminDiagnosticComponent getUserListByRoleName " + JSON.stringify(err));
            this.apiService.catchError(err);
          } catch (e) {
            console.log("Error Exception AdminDiagnosticComponent getUserListByRoleName " + e);
          }
        }
      );
  }
  ...
  updateUser(action: string, userId: string) {
    if (action == 'add') {
      this.router.navigate([
        this.routeConfig.diagnosticRegistrationPath
      ]);
    } else if (action == 'edit') {
      this.router.navigate([
        this.routeConfig.diagnosticRegistrationPath , userId
      ]);
    }
  }
          ...
          .subscribe(
            (res: any) => {
              try {
                ...
                console.log("Success Exception DiagnosticsComponent deleteUserDetail " + e);
              }
              this.stickyBarService.hideLoader("");
            },
            (err: any) => {
              try {
                this.stickyBarService.hideLoader("");
                console.log("Error DiagnosticsComponent deleteUserDetail " + JSON.stringify(err));
                this.apiService.catchError(err);
              } catch (e) {
                console.log("Error Exception DiagnosticsComponent deleteUserDetail " + e);
              }
            });
      }
    });
