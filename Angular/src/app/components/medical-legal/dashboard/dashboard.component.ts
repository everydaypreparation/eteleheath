...
import { AppointmentBookLaterModalComponent } from '../../appointment-book-later-modal/appointment-book-later-modal.component';
...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  appointmentBookClick(appointmentId: any, doctorId: any) {
    const dialogRef = this.dialog.open(AppointmentBookLaterModalComponent, {
      maxWidth: "800px",
      data: { "appointmentId": appointmentId, "doctorId": doctorId }
    });
  ...
  nextAppointmentClick(appointmentId: any) {
    this.router.navigate([
      this.routeConfig.medicalLegalPatientDetailsPath, appointmentId
    ]);
  }
  ...
  getDashboardStats(): void {
    this.stickyBarService.showLoader("");
    this.apiService
      .getWithBearer(this.apiConfig.getLegalStats + this.loginUserId)
      .subscribe(
        (res: any) => {
          try {
            if (res.result.statusCode == 200) {
              this.newCases = res.result.newCases;
              this.reportDue = res.result.reportDue;
