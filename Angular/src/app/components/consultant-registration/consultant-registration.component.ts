...
export class ConsultantRegistrationComponent implements OnInit {
  ...
  createUser(): void {
    ...
    this.apiService
      .postFormDataWithBearer(this.apiConfig.consultantCreate, formdata)
      .subscribe(
        (res: any) => {
          try {
            ...
            if (res.result.statusCode == 200) {
              ...
              this.router.navigate([this.routeConfig.adminConsultantsPath]);
            } else {
              this.stickyBarService.showErrorSticky(res.result.message);
            }
          } catch (e) {
            console.log("Success Exception ConsultantRegistrationComponent createUser " + e);
          }
        },
        (err: any) => {
          try {
            this.stickyBarService.hideLoader("");
            console.log("Error ConsultantRegistrationComponent createUser " + JSON.stringify(err));
      ...
      .subscribe(
        (res: any) => {
          try {
            ...
            console.log("Success Exception ConsultantRegistrationComponent updateUser " + e);
          }
        },
        (err: any) => {
          try {
            this.stickyBarService.hideLoader("");
            console.log("Error ConsultantRegistrationComponent updateUser " + JSON.stringify(err));
            this.apiService.catchError(err);
          } catch (e) {
            console.log("Error Exception ConsultantRegistrationComponent updateUser " + e);
          }
        }
      );
  }
      ...
      .subscribe(
        (res: any) => {
          ...
          try {
            ...
            console.log("Success Exception ConsultantRegistrationComponent getUserDetailsById " + e);
          }
          this.stickyBarService.hideLoader("");
        },
        (err: any) => {
          try {
            this.stickyBarService.hideLoader("");
            console.log("Error ConsultantRegistrationComponent getUserDetailsById " + JSON.stringify(err));
            this.apiService.catchError(err);
          } catch (e) {
            console.log("Error Exception ConsultantRegistrationComponent getUserDetailsById " + e);
          }
        }
      );
  }
