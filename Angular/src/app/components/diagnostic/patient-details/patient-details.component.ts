...
export class PatientDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.patientNumericId = this.activatedRoute.snapshot.params['patientNumericId'];
  ...
  getAppoinmentDetailsById(): void {
    this.stickyBarService.showLoader("");
    this.apiService
      .getWithBearer(this.apiConfig.GetInsuranceAppointmentDetailsById + this.appointmentId)
      .subscribe(
        (res: any) => {
          try {
            if (res.result.statusCode == 200) {
  ...
  patientRecordUploadClick() {
    const dialogRef = this.dialog.open(UploadDocumentModalComponent, {
      data: {"loginUserId": this.patientId}
    });
