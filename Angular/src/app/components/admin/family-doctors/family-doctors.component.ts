...
@Component({
  ...
  styleUrls: ['./family-doctors.component.scss']
})
export class FamilyDoctorsComponent implements OnInit {
  users: any[] = [];
  roleName: string = 'familydoctor';
  ...
  displayedColumns: string[] = ['id', 'fullName', 'cityId', 'stateId', 'countryId', 'phoneNumber', 'oncologySpecialty', 'action'];
  ...
  constructor(
    ...
    this.userService.setNav("familydoctors");
  }
      ...
      .subscribe(
        (res: any) => {
          try {
            ...
            console.log("Success Exception AdminFamilyDoctorComponent getUserListByRoleName " + e);
          }
        ...
        (err: any) => {
          try {
            this.stickyBarService.hideLoader("");
            console.log("Error AdminFamilyDoctorComponent getUserListByRoleName " + JSON.stringify(err));
            this.apiService.catchError(err);
          } catch (e) {
            console.log("Error Exception AdminFamilyDoctorComponent getUserListByRoleName " + e);
          }
        }
      );
  }
  ...
  updateUser(action: string, userId: string) {
    if (action == 'add') {
      this.router.navigate([
        this.routeConfig.familyDoctorRegistrationPath
      ]);
    } else if (action == 'edit') {
      this.router.navigate([
        this.routeConfig.familyDoctorRegistrationPath, userId
      ]);
    }
  }
          ...
          .subscribe(
            (res: any) => {
              try {
                ...
                console.log("Success Exception FamilyDoctorsComponent deleteUserDetail " + e);
              }
              this.stickyBarService.hideLoader("");
            },
            (err: any) => {
              try {
                this.stickyBarService.hideLoader("");
                console.log("Error FamilyDoctorsComponent deleteUserDetail " + JSON.stringify(err));
                this.apiService.catchError(err);
              } catch (e) {
                console.log("Error Exception FamilyDoctorsComponent deleteUserDetail " + e);
              }
            });
      }
    });
