...
export class SentComponent implements OnInit {
  ...
  ngOnInit(): void {
    $('.sendButton').addClass("selectedLeftBar");
  ...
  getAllUserEmailsByUserId(loginUserId: any): void {
    ...
    this.sentBody.userId = loginUserId;
    this.stickyBarService.showLoader("");
    this.apiService.postWithBearer(this.apiConfig.getAllSenderEmailsByUserId, this.sentBody)
      .subscribe(
        (res: any) => {
          try {
  ...
  viewMail(id: any){
    this.router.navigate([this.routeConfig.messagingViewMailPath, 'sent', id]);
  }
  ...
  deleteMailsByMailId() {
    ...
    dialogRef.afterClosed().subscribe(dialogResult => {
      if (dialogResult == true) {
        this.stickyBarService.showLoader("");
        this.apiService.deleteWithBearer(this.apiConfig.deleteMails + this.selectedMailIdsArray.toString()+'&IsDeleteReceiver=false&IsDeleteSender=true&IsDeleteTrash=false&IsDeleteDraft=false&UserId='+this.loginUserId)
          .subscribe(
            (res: any) => {
              try {
                if (res.result.statusCode == 200) {
