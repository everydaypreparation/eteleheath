...
import { RequestTestModalComponent } from 'src/app/request-test-modal/request-test-modal.component';
...
import { ViewConsultantReportModalComponent } from '../../view-consultant-report-modal/view-consultant-report-modal.component';
...
export class PatientDetailsComponent implements OnInit {
  ...
  getAllUpcomingAppointment(): void {
    ...
    this.apiService
      ...
      .getWithBearer(this.apiConfig.getBookedAppointmentByUserIdAndAppointmentId + this.loginUserId + "&AppointmentId=" + this.appointmentId)
      .subscribe(
        (res: any) => {
          try {
            if (res.result.statusCode == 200) {
              this.nextAppointment = res.result.items[0];
            ...
            console.log("Success Exception PatientDetailsComponent getAllUpcomingAppointment " + e);
          }
        ...
        (err: any) => {
          try {
            this.stickyBarService.hideLoader("");
            console.log("Error PatientDetailsComponent getAllUpcomingAppointment " + JSON.stringify(err));
            this.apiService.catchError(err);
          } catch (e) {
            console.log("Error Exception PatientDetailsComponent getAllUpcomingAppointment " + e);
          }
        }
      );
  }
      ...
      .subscribe(
        (res: any) => {
          try {
            ...
            console.log("Success Exception PatientDetailsComponent cancelAppointment " + e);
          }
          this.stickyBarService.hideLoader("");
        },
        (err: any) => {
          try {
            this.stickyBarService.hideLoader("");
            console.log("Error PatientDetailsComponent cancelAppointment " + JSON.stringify(err));
            this.apiService.catchError(err);
          } catch (e) {
            console.log("Error Exception PatientDetailsComponent cancelAppointment " + e);
          }
        }
      );
  }
        ...
        (res: any) => {
          try {
            if (res.result.statusCode == 200) {
              ...
              if (this.appointmentDetails && this.appointmentDetails.id && !this.appointmentDetails.id.includes('C-')) {
                this.getAllUserEmailsByUserId(this.loginUserId, this.appointmentDetails.patientId, false);
                this.getSelectedConsultantReportsByPatientIdAndDoctorId();
                this.getConsultantReports(this.appointmentDetails.patientId, false);
              } else {
                this.getAllUserEmailsByUserId(this.loginUserId, this.appointmentDetails.legalId, false);
                this.getConsultantReports(this.appointmentDetails.legalId, false);
              }
              ...
              if (this.appointmentDetails.doctorProfileUrl) {
                if (this.appointmentDetails.doctorProfileUrl.includes(this.apiConfig.matchesUrl)) {
                  this.familyProfileUrl = this.sanitizer.bypassSecurityTrustResourceUrl(`${this.appointmentDetails.doctorProfileUrl}`);
                } else {
                  this.familyProfileUrl = this.sanitizer.bypassSecurityTrustResourceUrl(`data:image/png;base64, ${this.appointmentDetails.doctorProfileUrl}`);
                }
                //this.familyProfileUrl = this.sanitizer.bypassSecurityTrustResourceUrl(`data:image/png;base64, ${this.appointmentDetails.doctorProfileUrl}`);
              }
              ...
              // if (this.appointmentDetails.legalProfileUrl) {
              //   if (this.appointmentDetails.legalProfileUrl.includes(this.apiConfig.matchesUrl)) {
              //     this.legalProfileUrl = this.sanitizer.bypassSecurityTrustResourceUrl(`${this.appointmentDetails.legalProfileUrl}`);
              //   } else {
              //     this.legalProfileUrl = this.sanitizer.bypassSecurityTrustResourceUrl(`data:image/png;base64, ${this.appointmentDetails.legalProfileUrl}`);
              //   }
              // }
            ...
            console.log("Success Exception PatientDetailsComponent getAppoinmentDetailsById " + e);
          }
          this.stickyBarService.hideLoader("");
        },
        (err: any) => {
          try {
            this.stickyBarService.hideLoader("");
            console.log("Error PatientDetailsComponent getAppoinmentDetailsById " + JSON.stringify(err));
            this.apiService.catchError(err);
          } catch (e) {
            console.log("Error Exception PatientDetailsComponent getAppoinmentDetailsById " + e);
          }
        }
      );
  }
  ...
  getDocumentListByAppointmentId(isConcat: boolean): void {
    ...
    let apiUrl = '';
    if (this.appointmentDetails && this.appointmentDetails.id && !this.appointmentDetails.id.includes('C-')) {
      apiUrl = this.apiConfig.getDocumentListByUserId + this.patientId+"&limit="+pageLimit+"&page="+pages;
    } else {
      apiUrl = this.apiConfig.getDocumentListByAppointmentId + this.appointmentId+"&limit="+pageLimit+"&page="+pages;
    }
    this.stickyBarService.showLoader("");
    this.apiService
  ...
  patientRecordUploadClick() {
    ...
    const dialogRef = this.dialog.open(UploadDocumentModalComponent, {
      data: { "loginUserId": id, "appointmentId": this.appointmentId }
    });
  ...
  composeMessage() {
    if (this.idForCompose) {
      sessionStorage.setItem('replyMailId', this.idForCompose);
  ...
  requestTest(): void {
    let name = this.appointmentDetails.firstName + ' ' + this.appointmentDetails.lastName;
    let pId = this.appointmentDetails.patientId;
    let id = this.appointmentDetails.id;
    let userId = this.user.id;
    const dialogRef = this.dialog.open(RequestTestModalComponent, {
      data: { patientName: name, appointmentDate: this.nextAppointment.slotStartTime, patientId: pId, consultantId: userId, id: id },
      autoFocus: false
    });
  ...
  getConsultantReports(id: any, isConcat: boolean): void {
    ...
    this.stickyBarService.showLoader("");
    this.apiService
      .getWithBearer(this.apiConfig.getConsultantReportForAllRole + this.envAndUrlService.UUID + "&DoctorId=" + this.loginUserId + "&PatientId=" + id + "&RoleName=" + this.user.roleNames[0]+"&limit="+pageLimit+"&page="+pages)
      .subscribe(
        (res: any) => {
          try {
            if (res.result.statusCode == 200) {
          ...
          .subscribe(
            (res: any) => {
              try {
                ...
                console.log("Success Exception PatientDetailsComponent deleteDocumentById " + e);
              }
              this.stickyBarService.hideLoader("");
            },
            (err: any) => {
              try {
                this.stickyBarService.hideLoader("");
                console.log("Error PatientDetailsComponent deleteDocumentById " + JSON.stringify(err));
                this.apiService.catchError(err);
              } catch (e) {
                console.log("Error Exception PatientDetailsComponent deleteDocumentById " + e);
              }
            });
      }
    });
  ...
  createReport() {
    this.router.navigate([
      this.routeConfig.consultantReportPath, this.appointmentDetails.consultId, this.appointmentId
    ]);
  }
  ...
  viewConsultantReport(documentId: any, documentName: any, documentDate: any) {
    const dialogRef = this.dialog.open(ViewConsultantReportModalComponent, {
      width: "100%",
      height: "100%",
      data: { "documentId": documentId, "doctorName": documentName, "docDate": documentDate }
    });
  ...
  getSelectedConsultantReportsByPatientIdAndDoctorId(): void {
    this.stickyBarService.showLoader("");
    this.apiService
      .getWithBearer(this.apiConfig.getIntakeConsultantReportsByPatientId + this.appointmentDetails.patientId + "&AppointmentId=" + this.appointmentId + "&DoctorId=" + this.envAndUrlService.UUID)
      .subscribe(
        (res: any) => {
          try {
            if (res.result.statusCode == 200) {
              ...
              for (let record of this.selectedConsultantReports) {
                ...
                // record.month = documentDate[1].substr(0, 3);
                record.documentName = record.documentName.split(".")[0];
                record.documentExtension = "assets/images/report-5.png";
              }
            }
          } catch (e) {
    ...
    if (e.target.scrollTop + e.target.clientHeight >= e.target.scrollHeight) {
      if (type == "patientRecords" && this.apisPages.patientRecords.more) {
        ...
        if (this.appointmentDetails && this.appointmentDetails.id && !this.appointmentDetails.id.includes('C-')) {
          ...
          this.getAllUserEmailsByUserId(this.loginUserId, this.appointmentDetails.legalId, true);
        }
      }else if (type == "consultantReport" && this.apisPages.consultantReport.more) {
        this.apisPages.consultantReport.page += 1;
        if (this.appointmentDetails && this.appointmentDetails.id && !this.appointmentDetails.id.includes('C-')) {
          this.getConsultantReports(this.appointmentDetails.patientId, true);
        } else {
          this.getConsultantReports(this.appointmentDetails.legalId, true);
        }
      }
    }
  }
