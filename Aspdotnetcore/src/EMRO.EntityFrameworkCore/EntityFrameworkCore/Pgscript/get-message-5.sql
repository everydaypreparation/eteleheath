...
-- DROP FUNCTION public."usp_GetMessages"(text, text,text,text);
...
CREATE OR REPLACE FUNCTION public."usp_GetMessages"(
	...
	secretkey text,
	algotype text)
    RETURNS TABLE(firstname text, lastname text, messageid uuid, subject text, messagestext text, senderuserids text, receiveruserids text, createdon timestamp without time zone, emailaddress character varying, readby text, appointmentid uuid) 
    LANGUAGE 'plpgsql'
...
select emro_sym_decrypt(us."Name",secretKey,algotype) as "FirstName",emro_sym_decrypt(us."Surname",secretKey,algotype) as "LastName",u."UserMessagesId" as "MessageId",
 u."Subject",u."MessagesText",u."SenderUserIds",u."ReceiverUserIds",u."CreatedOn",us."EmailAddress",u."ReadBy", u."AppointmentId"
