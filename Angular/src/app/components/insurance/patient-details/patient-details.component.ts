...
export class PatientDetailsComponent implements OnInit {
  ...
  getAllUpcomingAppointment(doctorId: any): void {
    this.stickyBarService.showLoader("");
    this.apiService
    .getWithBearer(this.apiConfig.getBookedAppointmentByUserIdAndAppointmentId + doctorId+"&AppointmentId="+this.appointmentId)
    .subscribe(
      (res: any) => {
        try {
  ...
  rescheduleClick(appointmentId: any, doctorId: any) {
    ...
    dialogRef.afterClosed().subscribe(dialogResult => {
      if (dialogResult == true) {
        this.getAllUpcomingAppointment(this.doctorId);
      }
    });
  }
  ...
  getAppoinmentDetailsById(): void {
    this.stickyBarService.showLoader("");
    this.apiService
      .getWithBearer(this.apiConfig.GetInsuranceAppointmentDetailsById + this.appointmentId+"&UserId="+this.loginUserId)
      .subscribe(
        (res: any) => {
          try {
            if (res.result.statusCode == 200) {
              ...
              this.doctorId = this.appointmentDetails.doctorId;
              this.getConsultantReports(this.appointmentDetails.doctorId, false);
              this.getAllUpcomingAppointment(this.appointmentDetails.doctorId);
              this.getAllUserEmailsByUserId(this.loginUserId, this.appointmentDetails.doctorId, false);
  ...
  getDocumentListByAppointmentId(isConcat: boolean): void {
    ...
    this.stickyBarService.showLoader("");
    this.apiService
      .getWithBearer(this.apiConfig.getDocumentListByAppointmentId+ this.appointmentId+"&limit="+pageLimit+"&page="+pages)
      .subscribe(
        (res: any) => {
          try {
            if (res.result.statusCode == 200) {
  ...
  patientRecordUploadClick() {
    const dialogRef = this.dialog.open(UploadDocumentModalComponent, {
      data: {"loginUserId": this.loginUserId, "appointmentId": this.appointmentId}
    });
  ...
  composeMessage() {
    if (this.doctorId) {
      sessionStorage.setItem('replyMailId', this.doctorId);
  ...
  getAllUserEmailsByUserId(loginUserId: any, doctorId: any, isConcat: boolean): void {
    let pageLimit = this.apisPages.userMessages.limit;
    let pages = (isConcat ? this.apisPages.userMessages.page : 1);
    this.stickyBarService.showLoader("");
    this.apiService.getWithBearer(this.apiConfig.getUserMessages + loginUserId + '&ToUserId=' + doctorId + '&appointmentId=' + this.appointmentId+"&limit="+pageLimit+"&page="+pages)
      .subscribe(
        (res: any) => {
          try {
            if (res.result.statusCode == 200) {
  ...
  getConsultantReports(doctorId: any, isConcat: boolean): void {
    ...
    this.stickyBarService.showLoader("");
    this.apiService
    .getWithBearer(this.apiConfig.getConsultantReportForAllRole+this.envAndUrlService.UUID+"&DoctorId="+doctorId+"&PatientId="+this.loginUserId+"&RoleName="+this.user.roleNames[0]+"&limit="+pageLimit+"&page="+pages)
      .subscribe(
        (res: any) => {
          try {
            if (res.result.statusCode == 200) {
  ...
  addNote() {
    ...
    const dialogRef = this.dialog.open(NotesModelComponent, {
      data: { "loginUserId": this.loginUserId, "noteId": "", "note": "", "appointmentId": this.appointmentId }
    });
  ...
  scrollHandler(e: any, type: string): void {
    if (e.target.scrollTop + e.target.clientHeight >= e.target.scrollHeight) {
      if (type == "patientRecords" && this.apisPages.patientRecords.more) {
        ...
        this.getAllUserEmailsByUserId(this.loginUserId, this.appointmentDetails.doctorId, true);
