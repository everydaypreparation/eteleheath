...
export class InboxComponent implements OnInit {
  ...
  ngOnInit(): void {
    $('.inboxButton').addClass("selectedLeftBar");
  ...
  getAllUserEmailsByUserId(loginUserId: any): void {
    ...
    this.apiService.postWithBearer(this.apiConfig.getAllUserEmailsByUserId, this.inboxBody)
      .subscribe(
        (res: any) => {
          try {
            ...
            console.log("Success Exception InboxComponent getAllUserEmailsByUserId " + e);
          }
        ...
        (err: any) => {
          try {
            this.stickyBarService.hideLoader("");
            console.log("Error InboxComponent getAllUserEmailsByUserId " + JSON.stringify(err));
            this.apiService.catchError(err);
          } catch (e) {
            console.log("Error Exception InboxComponent getAllUserEmailsByUserId " + e);
          }
        }
      );
  }
  ...
  viewMail(id: any) {
    this.viewUserMail(id);
    this.router.navigate([this.routeConfig.messagingViewMailPath, 'inbox', id]);
  }

  viewUserMail(id: any) {
    this.readByBody.userId = this.loginUserId;
    this.readByBody.messageId = id;
    this.stickyBarService.showLoader("");
    this.apiService.postWithBearer(this.apiConfig.readBy, this.readByBody)
      .subscribe(
        (res: any) => {
          try {
            // if (res.result.statusCode == 200) {
            ...
            console.log("Success Exception InboxComponent viewUserMail " + e);
          }
          this.stickyBarService.hideLoader("");
        },
        (err: any) => {
          try {
            this.stickyBarService.hideLoader("");
            console.log("Error InboxComponent viewUserMail " + JSON.stringify(err));
            this.apiService.catchError(err);
          } catch (e) {
            console.log("Error Exception InboxComponent viewUserMail " + e);
          }
        }
      );
  }
  ...
  verifyReadmail(ids: any) {
    if (ids) {
      return ids.includes(this.loginUserId);
    } else {
      return false;
    }
  }
  ...
  deleteMailsByMailId() {
    ...
    dialogRef.afterClosed().subscribe(dialogResult => {
      if (dialogResult == true) {
        this.stickyBarService.showLoader("");
        this.apiService.deleteWithBearer(this.apiConfig.deleteMails + this.selectedMailIdsArray.toString() + '&IsDeleteReceiver=true&IsDeleteSender=false&IsDeleteTrash=false&IsDeleteDraft=false&UserId=' + this.loginUserId)
          .subscribe(
            (res: any) => {
              try {
                if (res.result.statusCode == 200) {
