...
export class ConsultantsComponent implements OnInit {
  ...
  constructor(private router: Router,
    ...
    this.userService.setNav("consultants");
  }
      ...
      .subscribe(
        (res: any) => {
          try {
            ...
            console.log("Success Exception AdminConsultantComponent getUserListByRoleName " + e);
          }
        ...
        (err: any) => {
          try {
            this.stickyBarService.hideLoader("");
            console.log("Error AdminConsultantComponent getUserListByRoleName " + JSON.stringify(err));
            this.apiService.catchError(err);
          } catch (e) {
            console.log("Error Exception AdminConsultantComponent getUserListByRoleName " + e);
          }
        }
      );
  }
  ...
  updateUser(action: string, userId: string) {
    if (action == 'add') {
      this.router.navigate([
        this.routeConfig.consultantRegistrationPath
      ]);
    } else if (action == 'edit') {
      this.router.navigate([
        this.routeConfig.consultantRegistrationPath, userId
      ]);
    }
  }
          ...
          .subscribe(
            (res: any) => {
              try {
                ...
                console.log("Success Exception ConsultantsComponent deleteUserDetail " + e);
              }
              this.stickyBarService.hideLoader("");
            },
            (err: any) => {
              try {
                this.stickyBarService.hideLoader("");
                console.log("Error ConsultantsComponent deleteUserDetail " + JSON.stringify(err));
                this.apiService.catchError(err);
              } catch (e) {
                console.log("Error Exception ConsultantsComponent deleteUserDetail " + e);
              }
            });
      }
    });
